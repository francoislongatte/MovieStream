...
import "./Movies.scss";
import { MOVIESBYGENRE } from "../../../redux/reducers/movieDb";
...
class Movies extends Component {
    ...
    componentWillMount() {
        if (this.props.gender.length > 0) this.props.movieApi.searchMoviesByGender(this.props.gender[0].id);
    }
    ...
    componentDidUpdate(prevProps) {
        if (JSON.stringify(this.props.gender) !== JSON.stringify(prevProps.gender)) {
            ...
            if (this.props.movies.length === 0) {
                this.props.movieApi.searchMoviesByGender(this.props.gender[0].id)
            }
        }
    ...
    handleSubmit(event) {
        this.props.movieApi.searchMoviesByGender(this.state.valueOptions);
    }
    ...
    render() {
        return (
            <>
                ...
                <View videos={this.state.movies} listName={MOVIESBYGENRE} genderTitle={this.state.title} />
            </>
...
const mapStateToProps = state => {
    return {
        movies: state.movieDb.MOVIESBYGENRE,
        gender: state.movieDb.genres.movie
    };
}
